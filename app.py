...
Сохтор:
  app.py
  requirements.txt
  templates/        ← HTML файлҳо
  static/
    style.css
    js/api.js
    uploads/videos/ ← Видеоҳо
  instance/
    platform.db     ← SQLite база
"""
...
# ===== APP SETUP =====
app = Flask(__name__,
            template_folder='templates',   # HTML файлҳо
            static_folder='templates')        # CSS, JS, видео

app.secret_key = os.environ.get('SECRET_KEY', 'platformtj-dev-secret-2024')
...
def init_db():
    ...
    if not db.execute("SELECT 1 FROM users WHERE username='student1'").fetchone():
        db.execute(
            "INSERT INTO users (name,username,password,role,avatar,points) VALUES (?,?,?,?,?,?)",
            ('Алишер Раҳимов','student1',
             generate_password_hash('pass123'),'student','👨‍🎓', 850))
    ...
    if not db.execute("SELECT 1 FROM courses LIMIT 1").fetchone():
        db.execute(
            ...
            ('Асосҳои Python','Барномасозии Python аз сифр то касбӣ.',
             '🐍','Ибтидоӣ','40 соат','programming'))
...
# =====================================================
# ===== СТРАНИЦАҲО (HTML) — render_template =====
# =====================================================
...
@app.route('/reyting')
def page_reyting():
    return render_template('reyting.html')
...
def create_user():
    ...
    try:
        ...
        db.close()
        return jsonify({'error': 'Ин логин аллакай мавҷуд аст!'}), 409
...
def update_user(uid):
    ...
    if d.get('password'):
        fields.append("password=?"); vals.append(generate_password_hash(d['password']))
    if not fields:
        db.close(); return jsonify({'error': 'Тағйирот нест'}), 400
    vals.append(uid)
...
def delete_user(uid):
    if uid == 1:
        return jsonify({'error': 'Супер-администраторро ҳазф кардан мумкин нест!'}), 403
    db = get_db()
...
def get_courses():
    db = get_db()
    courses = rows2list(db.execute("SELECT * FROM courses ORDER BY id").fetchall())
    for c in courses:
...
def create_course():
    d = request.get_json()
    if not d.get('title'):
        return jsonify({'error': 'Ном ҳатмӣ!'}), 400
    db = get_db()
    cur = db.execute(
        "INSERT INTO courses (title,description,icon,level,duration,category) VALUES (?,?,?,?,?,?)",
...
def update_course(cid):
    ...
    db = get_db()
    db.execute(
        "UPDATE courses SET title=?,description=?,icon=?,level=?,duration=?,category=? WHERE id=?",
...
def delete_course(cid):
    db = get_db()
    for t in rows2list(db.execute(
            "SELECT video_path FROM topics WHERE course_id=?", (cid,)).fetchall()):
...
def get_topics():
    ...
    db  = get_db()
    if cid:
        topics = rows2list(db.execute(
            "SELECT * FROM topics WHERE course_id=? ORDER BY order_num", (cid,)).fetchall())
    else:
        topics = rows2list(db.execute(
            "SELECT * FROM topics ORDER BY course_id,order_num").fetchall())
    db.close()
    for t in topics: t.pop('video_path', None)
...
def create_topic():
    ...
    if 'video' in request.files:
        ...
        if f and f.filename and allowed_video(f.filename):
            ...
            uname = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{fn}"
            vpath = os.path.join(UPLOAD_DIR, uname)
            f.save(vpath); vname = fn
    db  = get_db()
    cur = db.execute(
        "INSERT INTO topics (course_id,title,description,order_num,video_name,video_path) VALUES (?,?,?,?,?,?)",
        (course_id, title, desc, order, vname, vpath))
    db.commit()
    t = row2dict(db.execute("SELECT * FROM topics WHERE id=?", (cur.lastrowid,)).fetchone())
    db.close(); t.pop('video_path', None)
...
def update_topic(tid):
    ...
    vname = old['video_name']; vpath = old['video_path']
    if 'video' in request.files:
        f = request.files['video']
        if f and f.filename and allowed_video(f.filename):
            if vpath and os.path.exists(vpath): os.remove(vpath)
            fn    = secure_filename(f.filename)
            uname = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{fn}"
            vpath = os.path.join(UPLOAD_DIR, uname)
            f.save(vpath); vname = fn
    db.execute(
        "UPDATE topics SET title=?,description=?,order_num=?,course_id=?,video_name=?,video_path=? WHERE id=?",
        (request.form.get('title', old['title']),
         request.form.get('description', old['description']),
         request.form.get('order_num', old['order_num'], type=int),
         request.form.get('course_id', old['course_id'], type=int),
         vname, vpath, tid))
    db.commit(); db.close()
    return jsonify({'success': True})
...
def delete_topic(tid):
    ...
    if t and t['video_path'] and os.path.exists(t['video_path']):
        os.remove(t['video_path'])
...
def stream_video(tid):
    db = get_db()
    t  = row2dict(db.execute(
        "SELECT video_path FROM topics WHERE id=?", (tid,)).fetchone())
    db.close()
    if not t or not t['video_path']:
        return jsonify({'error': 'Видео нест'}), 404
...
def get_tests():
    ...
    db  = get_db()
    if cid:
        tests = rows2list(db.execute(
            "SELECT * FROM tests WHERE course_id=? ORDER BY id", (cid,)).fetchall())
    else:
        tests = rows2list(db.execute("SELECT * FROM tests ORDER BY id").fetchall())
    db.close()
    for t in tests:
...
def create_test():
    ...
    db.close(); t['questions'] = json.loads(t['questions'])
    return jsonify(t), 201
...
def update_test(tid):
    ...
    db = get_db()
    db.execute(
        "UPDATE tests SET title=?,description=?,course_id=?,time_limit=?,questions=? WHERE id=?",
...
def submit_test(tid):
    ...
    uid     = session['user_id']
    db.execute("""INSERT INTO results (user_id,test_id,score,total,percent)
                  VALUES (?,?,?,?,?)
                  ON CONFLICT(user_id,test_id) DO UPDATE SET
                  score=excluded.score,total=excluded.total,
                  percent=excluded.percent,date=datetime('now')""",
...
def get_tasks():
    cid = request.args.get('course_id')
    db  = get_db()
    tasks = rows2list(db.execute(
        "SELECT * FROM tasks WHERE course_id=? ORDER BY id" if cid else
        "SELECT * FROM tasks ORDER BY id",
        (cid,) if cid else ()).fetchall())
    db.close(); return jsonify(tasks)
...
def update_task(tid):
    ...
    db = get_db()
    db.execute(
        "UPDATE tasks SET title=?,description=?,course_id=?,difficulty=?,language=?,starter_code=? WHERE id=?",
        (d['title'],d.get('description',''),d['course_id'],
...
def leaderboard():
    ...
    for i,u in enumerate(users):
        u['rank'] = i+1; u['avg_percent'] = round(u['avg_percent'])
    return jsonify(users)
...
def get_schedule():
    db = get_db()
    items = rows2list(db.execute(
        "SELECT * FROM schedule ORDER BY day_of_week,time").fetchall())
...
def create_schedule():
    ...
    db  = get_db()
    cur = db.execute(
        "INSERT INTO schedule (title,type,day_of_week,time,duration,repeat) VALUES (?,?,?,?,?,?)",
...
if __name__ == '__main__':
    init_db()
    print("=" * 55)
    print("  ПлатформаТЈ — Flask Backend")
    print("  Суроға:  http://localhost:5000")
